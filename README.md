...
## Components
- `index.php` - fullscreen viewer that discovers JPG/JPEG/PNG/GIF/WEBP files from `uploads/`, sorts them naturally (or shuffles them when randomization is enabled), and cycles through them at the configured interval. When the playlist wraps it forces a page reload so newly arrived images appear automatically and a new shuffle is applied. **Images that have existed longer than the retention window are deleted on the fly.**
- `upload.php` - upload console with a drag-and-drop zone plus settings form. It enforces extension + MIME checks, sanitizes filenames, avoids collisions, and persists `interval` (seconds between slides), `expire_days` (auto-delete window), and `randomize_order` (toggle shuffling the slideshow each cycle) to `settings.json`.
...
## Settings & data files
- `settings.json` - JSON document written by `upload.php` that stores `interval` (1-3600 seconds), `expire_days` (1-3650 days), and `randomize_order` (boolean toggle for shuffling the slideshow each cycle). The viewer and management panel honor these values. File will be created if not present.
